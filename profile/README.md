<div align="center">

<img src="https://github.com/OfficialNekoTeam.png" alt="NekoTeam" width="120" />

# Neko Team

</div>

Neko Team 是一个以 `AI` 、`AI Agent` 与 `Large Language Model` 为核心的开源开发组织，旗下包含一系列以 `Neko` 命名的项目。
致力于将实验性的想法 将架构设计、实用主义贯彻到底，让其改善开发者与用户在日常使用中的体验。

---

## 关于这个组织

**本组织相关域名**
- nekoteam.cc    neko团队 
- gitneko.com    neko团队的git仓库
- nekobot.dev    nekobot相关文档以及生态
- leolang.org    一门编程开发语言
- likoai.net    (新架构的玲可AI)  

## 项目文档

📚 **NekoBot**
- [https://docs.nekobot.dev](https://docs.nekobot.dev)
- [https://officialnekoteam.github.io (Github Pages)](https://officialnekoteam.github.io)
- [https://nekobot-docs.vercel.app (Vercel)](https://nekobot-docs.vercel.app)

**官方社群**

- QQ
    - [玲可猫猫的大学喵(插件开发者群聊)](https://qm.qq.com/q/UJVig4ury8)
    - [玲可猫猫的大学 1群](https://qm.qq.com/q/cakIFXheeY)
- QQ频道
    - [Neko猫猫的小窝](https://pd.qq.com/s/de131bcm9)
- 飞书
    - [开发者群聊](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=3d2gb6d5-1044-4376-9d84-d3c207007fce)
    - [社区意见等提议](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=f76s0727-817c-4105-888c-69e00ee36a27)

<div align="center">

**如果本组织仓库中的项目对你有帮助，欢迎前来 Star ⭐或是参与项目贡献**

</div>

> Joyful Radiant Xpanse 逐光而行，荣途浩瀚,愿我们给开发者们铺下的路越来越宽，越来越辽阔
